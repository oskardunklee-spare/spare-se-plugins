...
- [ ] **Quoted-requirement opener (Rule 28).** Flag any comment whose opener matches `Spare [verb] ["'\u201C\u2018]` — i.e., `Spare addresses "..."`, `Spare supports '...'`, `Spare handles "..."`, `Spare covers "..."`, `Spare provides '...'`, using any quote style. The quoted content is almost always the requirement text pasted back. Block. Implementation: `re.match(r'^Spare\\s+\\w+\\s+[\"\\'\u201C\u2018]', comment)`. If the quoted content is genuinely something else (a product feature name, a specific Spare term like \"Live Map\"), the SE can acknowledge and pass; otherwise the row must be rewritten without the quoted preamble.
- [ ] **Colon-intro filler construction (Rule 28).** Flag any comment where the first sentence restates the requirement, followed by a colon, followed by the actual answer, e.g. *"Spare addresses X in Spare EAM: supports Y."* Heuristic: if the first clause (up to the first colon) contains 6+ words AND the clause after the colon reads as a complete substantive answer, flag. This pattern signals the first clause is filler and the colon introduces the real answer. The fix is to delete the preamble + colon and just write the answer. Implementation: extract first_clause_before_colon and after_colon text; flag if len(first_clause.split()) >= 6 AND after_colon starts with a lowercase verb or "the" / "a" (i.e., reads as the continuation of a sentence, not a list intro).
- [ ] **Third-party system name not in deal context (Rule 24).** Applies to every integration category (ERP, fare, scheduling, telematics, GIS, CRM, ITSM, payment, etc.), not just ERPs. Scan every filled comment for named third-party systems. Compare against the loaded deal-context artifact. Any system named in a comment but not in the deal context is a blocker; the row must be re-drafted with the default integration framing (*"the agency's ERP / telematics provider / fare system / etc."*) or with the correct name if the SE confirms it belongs. Maintain this list of names to scan for (case-insensitive, match whole-word), grouped by category so new entries land in the right bucket:
    - **ERP / GL:** `Microsoft Dynamics 365`, `Dynamics 365`, `Dynamics`, `Workday`, `SAP`, `NetSuite`, `PeopleSoft`, `Oracle`
    - **Scheduling / paratransit:** `Trapeze`, `Ecolane`, `RouteMatch`, `myAvail`, `Avail`
    ...
    - **Implementation partners:** `Crowe`, `Deloitte`, `Accenture`
    
    If new integrations come up in real runs, add them to the relevant category. Implementation: for each comment, regex-search for each name (case-insensitive, word-boundary); for each hit, check whether the same name appears in the current `<agency>-deal-context.md` content. Report mismatches with row number + named system + its category.
...
- [ ] **Triggered corroboration for staleness and product-change risk (Rule 25).** For each row, detect whether any of these triggers fired:
    - Top precedent similarity < 0.5 (read from Internal Reasoning)
    - All cited precedents more than 12 months old (check year; if all precedents' year < current year - 1, trigger fires)
    - Requirement text contains any keyword from these groups (case-insensitive): `integration`, `interoperability`, `API`, `interface`, `exchange`, `feed`, `SSO`, `webhook`, `connector`, `import`, `export`, `security`, `compliance`, `audit`, `SOC`, `HIPAA`, `PCI`, `FIPS`, `PII`, `encryption`, `access control`, `penetration`, `eligibility`, `ADA`, `roadmap`, `future`, `planned`, `upcoming`, `next release`, `coming soon`, `competitor`, `competitive`
  
  When a trigger fired, Internal Reasoning must cite at least one corroborating source beyond the precedent: a Spare docs URL (mintlify.com / mintlify.io / docs.spare.com paths under `/sales-enablement`, `/customer-enablement`, `/internal-changelog`), a Glean result URL, or a Notion page URL. Rows where a trigger fired but only a precedent was cited are blockers; the fill skill must re-draft with docs / Glean corroboration, OR mark the row `I` (Need More Info). Report offending row numbers and which trigger fired.
- [ ] **Spare must appear within the first 5-7 words of the opening clause** (loosened in v1.4.1 from "first word"). Acceptable patterns: `Spare [Product] [verb]`, `Spare's [feature] [verb]`, `Within Spare [Product], [subject]...`, `In Spare's [feature], ...`, `Each [entity] in Spare [Product]...`, `[Entity] in Spare [Product] [verb]`, `Through Spare's [feature], ...`. Not acceptable: `The platform supports...`, `The system does...`, `Out of the box, ...`, `Yes, ...`, `Barcode scanning supports...`, or any opener where `Spare` doesn't appear in the first ~7 words. Flag.
- [ ] **Monotonous openers (v1.4.1).** For each section in the matrix, scan consecutive filled rows. If any 3+ consecutive rows share the same first 2 words (e.g., three rows in a row all opening with "Spare EAM"), the matrix reads as a stamped template even when each individual comment is correct. This is a blocker. The fix is to rewrite the second or third row using one of the alternate opener patterns from `voice-templates.md` "Opener variety" section. Implementation: `for each section, walk rows; track previous 2 openers; if row[i].first_2_words == row[i-1].first_2_words == row[i-2].first_2_words, flag rows i-1 and i for opener rewrite`. Report sections that triggered, the offending row range, and the repeated opener phrase.
- [ ] **Broken opener grammar (Rule 27).** Specifically flag any draft starting with `Spare's ` immediately followed by a preposition (`within`, `in`, `at`, `on`, `for`, `with`, `regarding`, `about`, `by`, `through`). That construction isn't grammatical ("Spare's within Spare EAM, X is covered..."). Block and require rewrite. Implementation: `re.match(r"^Spare's\\s+(within|in|at|on|for|with|regarding|about|by|through)\\b", comment, re.IGNORECASE)`.
- [ ] **Internal-note leakage from precedent (Rule 26).** Scan each drafted comment for markers that indicate scratch-note text was carried forward from the precedent corpus:
    - Literal `TBD`, `TBD:`, `TBD -`
    - Question marks inside the comment body (except if the requirement literally asks for clarification)
    - Hedging phrases: `might have complexity`, `might not want`, `not likely to have`, `requires decision`, `need custom`, `could use asset types`, `may need custom`
    - Obvious typos or all-caps shorthand: `Usse`, mid-sentence `SW`, `HW`, or similar undefined abbreviations
    - Meta-commentary phrases: `Specifically, custom Field`, `Specifically, TBD`
  
  Any match is a blocker. The fix is to re-draft the row using the precedent's factual content, not the precedent's scratch notes. Implementation: case-insensitive regex over the full list. Report the offending row + the exact marker found.
- [ ] **Meaningless hedge phrases.** Flag any comment containing: `with the caveat noted`, `with one noted sub-feature`, `with one scoped sub-feature flagged`, `sub-feature flagged`, `is covered with the caveat`. These are Claude-invented filler that reads as vague and defensive. Re-draft with specific language from the precedent.
- [ ] **Unsourced roadmap claims.** If a comment ends with `This is tracked on Spare's product roadmap` (or close variant), verify the Internal Reasoning cites a docs page or precedent that explicitly confirms roadmap status. If only a regular precedent is cited (not one that discusses roadmap), the claim is unsourced and is a blocker: roadmap commitments can become contractual obligations.
...
- [ ] Flag comments containing banned phrases: `leverage`, `leveraging`, `cutting-edge`, `best-in-class`, `seamlessly`, `robust and scalable`, `we pride ourselves`, `powerful platform`, `elevate`. (Promoted from style-drift to blocker in v1.4.0; voice violations are blockers now.)
- [ ] **Vendor-register phrases (Rule 28+ blocker in v1.4.0).** Blocker. Flag comments containing any of: `fully documented`, `typically owned`, `typically maintained`, `typically handled by`, `is reflected in`, `is passed to`, `is exchanged with`, `comprehensively`, `robustly`, `the platform exposes` (if it appears in more than one row of the same sheet). These are the markers that pull drafts toward generic enterprise-vendor voice and away from Spare's tone. If the `enforce-spare-voice` skill ran on the file (it should have, per fill-rfp-matrix's Finishing step), these shouldn't appear. When they do, the fix is to re-invoke `enforce-spare-voice` or rewrite the offending rows inline per the before/after examples in `voice-templates.md` "Spare's voice" section.
