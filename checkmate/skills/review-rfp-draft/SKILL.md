...
- [ ] **Vendor-register phrases (v1.3.8).** Flag comments containing any of: `fully documented`, `typically owned`, `typically maintained`, `typically handled by`, `is reflected in`, `is passed to`, `is exchanged with`, `comprehensively`, `robustly`, `the platform exposes` (if it appears in more than one row of the same sheet). These are the markers that pull drafts toward generic enterprise-vendor voice and away from Spare's tone. Recommend rewriting with concrete verbs per the before/after examples in `voice-templates.md` "Spare's voice" section.
